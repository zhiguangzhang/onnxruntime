...
# TODO: With @torch.jit.script, pytorch exporter converts float literals (1.41421) to Constant(float64). 
# This breaks ONNX spec. because the rest of the model has dtype being float32.
# @torch.jit.script
def f_gelu(x):
...
def bias_gelu(bias, y):
    x = bias + y
    return  x * 0.5 * (1.0 + torch.erf(x / 1.41421))    # [ONNXRuntimeError] : 1 : GENERAL ERROR : Type Error: Type parameter (T) bound to different types (tensor(float) and tensor(double) in node (Div_134)
    # return  x * np.float32(0.5) * (np.float32(1.0) + torch.erf(x / np.float32(1.41421)))    # raise OSError('could not find class definition')
    # return  x * torch.tensor(0.5, dtype=torch.float32) * (torch.tensor(1.0, dtype=torch.float32) + torch.erf(x / torch.tensor(1.41421, dtype=torch.float32)))   #[ONNXRuntimeError] : 1 : GENERAL ERROR : Type Error: Type parameter (T) bound to different types (tensor(float) and tensor(double) in node (Div_135).
...
# TODO: Custom op is not supported by PyTorch ONNX exporter and ORT.
# try:
#     import apex
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, use_ort_with_workaround=False):
        ...
        # TODO: revove this after pytorch exporter is fixed
        self.use_ort_with_workaround = use_ort_with_workaround
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, checkpoint_activations=False):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if self.use_ort_with_workaround:
                # TODO: PyTorch ONNX exporter cannot handle gather with integer input - triggered with 
                # loss_fct = CrossEntropyLoss(ignore_index=ignored_index). Undo the workaround once it is fixed.
                loss_fct = CrossEntropyLoss()
            else:
