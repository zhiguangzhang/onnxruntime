...
def glue_fine_tune_model_description():
    vocab_size = 30528
    ...
    label_ids_desc = IODescription('label_ids', ['batch',], torch.int64, num_classes = 2)
    loss_desc = IODescription('loss', [], torch.float32)
    # probability_desc = IODescription('probability', ['batch', 10], torch.float32)

    return ModelDescription([input_ids_desc, input_mask_desc, segment_ids_desc, label_ids_desc], [loss_desc])
...
def main():
    ...
    ################################################
    if args.use_ort:
        model_desc = glue_fine_tune_model_description()
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if args.use_ort:
                    ...
                    loss = model(input_ids, segment_ids, input_mask, label_ids)
                # ###########################################
                # onnx_file = '/bert_ort/liqun/test_out/glue_result/bert_fine_tune_glue.onnx'
                # torch.onnx._export(model, (input_ids, segment_ids, input_mask, label_ids), onnx_file,
                #                 input_names=['input_ids', 'segment_ids', 'input_mask', 'label_ids'], 
                #                 output_names=['loss',],
                #                 opset_version=10,
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                # TODO: ort_session to evaluate according to available inputs.
