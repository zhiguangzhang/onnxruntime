...
from ort_trainer import FuseSofmaxNLLToSoftmaxCE, create_ort_training_session_bind_parameters
from ort_trainer import ORTTrainer, ORTModel, IODescription, ModelDescription, create_ort_training_session_bind_parameters
from ort_trainer import ort_training_session_run_helper

def squad_fine_tune_model_description():
    vocab_size = 30528
    ...
    start_positions_desc = IODescription('start_positions', ['batch',], torch.int64, num_classes = 2)
    end_positions_desc = IODescription('end_positions', ['batch',], torch.int64, num_classes = 2)

    loss_desc = IODescription('loss', [], torch.float32)
    # probability_desc = IODescription('probability', ['batch', 10], torch.float32)

    return ModelDescription([input_ids_desc, segment_ids_desc, input_mask_desc, start_positions_desc, end_positions_desc], [loss_desc])
...
def main():
    ...
    parser.add_argument('--use_ort',
                        default = False,
                        action = 'store_true',
                        help="Whether to run ort.")
    ...
    parser.add_argument('--schedule',
                        ...
                        type=str)
    parser.add_argument('--use_ort_trainer_mixed_precision',
                        default=False,
                        action='store_true',
                        help="Whether to run with ort trainer mixed precision.")
    ...
    use_ort_with_workaround_for_onnx_gather = True
    model = BertForQuestionAnswering(config, use_ort_with_workaround_for_onnx_gather)
    ...
    ##############################################
    if args.use_ort:
        model_desc = squad_fine_tune_model_description()
        model = ORTModel(model, None, model_desc, device, postprocess_model, gradient_accumulation_steps=args.gradient_accumulation_steps)
    ##############################################
        # Prepare optimizer
    ...
    if args.do_train:
        if args.fp16 and not args.use_ort:
            try:
    ...
    if not args.use_ort:
        if args.local_rank != -1:
            ...
            # print('skip model = torch.nn.DataParallel(model) call in order to export onnx model')    
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                # ###########################################
                # onnx_file = '/bert_ort/liqun/test_out/squad_results/bert_fine_tune_squad.onnx'
                # torch.onnx._export(model, (input_ids, segment_ids, input_mask, start_positions, end_positions), onnx_file,
                #                 input_names=['input_ids', 'segment_ids', 'input_mask', 'start_positions', 'end_positions'], 
                #                 output_names=['loss',],
                #                 opset_version=11,
                #                 #dynamic_axes=dynamic_axes,
                #                 training=True,
                #                 _retain_param_name=True,
                #                 example_outputs=[loss,])
                # ###########################################
    ...
    if not args.use_ort:
        # TODO: implemente ORTModel save and load methods 
        if args.do_train:
